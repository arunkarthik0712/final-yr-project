...
if ($_SERVER["REQUEST_METHOD"]=="POST") {
  ...
  if (isset($_POST['btn'])) {
    $degree = $_POST['degree'];
    $degree_name = $_POST['dname'];
    $branch = $_POST['branch'];
    $university_institution = $_POST['univ'];
    $year_of_passing = $_POST['year'];
    $percentage_cgpa = $_POST['percent'];
    $class = $_POST['class'];
    $rank = $_POST['rank'];
        
    $stmt = $conn->prepare("INSERT INTO academic_details (`user_id`, `degree`, `degree_name`, `branch`,`university_institution`, `year_of_passing`, `percentage_cgpa`, `class`, `rank`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");


    $stmt->bind_param("issssssss", $user_id, $degree, $degree_name, $branch, $university_institution, $year_of_passing, $percentage_cgpa, $class, $rank);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE academic_details SET `degree`=?,`degree_name`=?, `branch`=?, `university_institution`=?, `year_of_passing`=?, `percentage_cgpa`=?, `class`=?, `rank`=? WHERE `user_id`=? AND `academic_id`=?");

        $updateStmt->bind_param("ssssssssii",$degree, $degree_name, $branch, $university_institution, $year_of_passing, $percentage_cgpa, $class, $rank, $user_id , $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `academic_details` WHERE `user_id`=? AND `academic_id`=?");
...
<br>
                  ...
                  <center><h3>Academic Details</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <input name="dname" type="text" class="form-control" id="dname" required>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="branch">*Branch</label>
            <input name="branch" type="text" class="form-control" id="branch" required>(eg:tamil,maths,etc..)
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="univ">University/institution</label>
            <input name="univ" type="text" class="form-control" id="univ" required>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
        ...
        <div class="col-md-4">
          <div class="form-group">
            <label for="percent">Percentage/CGPA</label>
            <input name="percent" type="text" class="form-control" id="percent" required>
            <br>
          </div>
        </div>
        <div class="col-md-6">
          <div class="form-group">
            ...
            <input name="class" type="text" class="form-control" id="class" required>
          </div>
        </div>
        <div class="col-md-6">
          <div class="form-group">
            <label for="rank">Rank(if any..)</label>
            <input name="rank" type="text" class="form-control" id="rank">
            <br>
          </div>
        </div>
        <div class="row">
        <div class="col-md-6">
          <div class="form-group">
            ...
            <a href="research.php" class="btn btn-dark" role="button">Next</a>
            <br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM academic_details WHERE user_id = '$user_id' ORDER BY year_of_passing DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['branch'];?></td>
                    <td><?=$row['university_institution'];?></td>
                    <td><?=$row['year_of_passing'];?></td>
                    <td><?=$row['percentage_cgpa'];?></td>
                    <td><?=$row['class'];?></td>
                    <td><?=$row['rank'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['academic_id'];?>">
