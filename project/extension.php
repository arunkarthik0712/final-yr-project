...
$stmt = $conn->prepare("INSERT INTO `extension_activities`(`user_id`, `position_designation`, `period_tenure`, `till_date`, `program_title`, `target_group`, `place`, `amount_spent`, `date_from`, `date_to`) VALUES (?,?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("isssssssss", $user_id, $position, $period, $tilldate, $title, $target,$place,$amount,$fdate,$todate);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `extension_activities` SET `position_designation`=?,`period_tenure`=?,`till_date`=?,`program_title`=?,`target_group`=?,`place`=?,`amount_spent`=?,`date_from`=?,`date_to`=?
            WHERE `user_id`=? AND `extension_id`=?");

        $updateStmt->bind_param("sssssssssii",$position, $period, $tilldate, $title, $target,$place,$amount,$fdate,$todate,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `extension_activities` WHERE `user_id`=? AND `extension_id`=?");
        ...
        <nav class="col-md-3 d-none d-md-block glass-sidebar ">
            <div class="sidebar-sticky" >
                ...
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="dropdownMenuLink" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fa fa-bars" aria-hidden="true"></i> EXPERIENCE/<br>CERTIFICATE/<br>CONFERENCE
                        </a>
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                    ...
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" id="dropdownMenuLink" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        <i class="fa fa-bars" aria-hidden="true"></i> RESEARCH/<br>JOURNAL/<br>BOOK
                        </a>
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
...
<br>
                  ...
                  <center><h3>Extension/Part V Activities Undertaken</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-3">
          <div class="form-group">
            <label for="period">Period/Tenure</label>
        ...
        <div class="col-md-2">
          ...
          <br>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="checkbox" id="tilldate" name="tilldate" value="Till Date">
              <label class="form-check-label" for="tilldate">Till Date</label>
            </div>
          </div>
        </div>
        <div class="col-md-3">
		...
		<div class="col-md-4">
          <div class="form-group">
            <label for="amount">Amount Spent(If Any)</label>
		...
		<div class="col-md-6">
          <div class="form-group">
            ...
            <input name="fdate" type="date" class="form-control" id="fdate" required>
            <br><br>
          </div>
        </div>
        <div class="col-md-6">
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="resource.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM extension_activities WHERE user_id = '$user_id'";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['date_to'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['extension_id'];?>">
      ...
      <div class="modal-body">
        ...
        <div class="col-md-2">
          ...
          <br>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="checkbox" id="tilldate" name="tilldate" value="Till Date">
              <p class="form-check-label">Till Date</p>
            </div>
          </div>
        </div>
        <div class="col-md-3">
