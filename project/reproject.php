...
$stmt = $conn->prepare("INSERT INTO `research_project`(`user_id`, `title_of_project`, `sponsoring_agency`, `duration`, `amount`, `project_type`, `status`, `date_from`, `date_to`) VALUES (?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("issssssss", $user_id, $title, $agency, $duration, $amount,$type,$status,$fdate,$todate);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `research_project` SET `title_of_project`=?,`sponsoring_agency`=?,`duration`=?,`amount`=?,`project_type`=?,`status`=?,`date_from`=?,`date_to`=?
            WHERE `user_id`=? AND `project_id`=?");

        $updateStmt->bind_param("ssssssssii",$title, $agency, $duration, $amount,$type,$status,$fdate,$todate,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `research_project` WHERE `user_id`=? AND `project_id`=?");
...
<br>
                  ...
                  <center><h3>Research Projects(Minor/Major) Undertaken With Details</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-3">
          <div class="form-group">
            <label for="agency">Sponsoring Agency</label>
            <input name="agency" type="text" class="form-control" id="agency" >
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
		...
		<div class="col-md-4">
          <div class="form-group">
            ...
            <input name="fdate" type="text" class="form-control" id="fdate" required>
            <br><br>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            ...
            <input name="todate" type="text" class="form-control" id="todate" required>
            <br><br>
          </div>
        </div>
		<div class="row">
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM research_project WHERE user_id = '$user_id' ORDER BY date_from DESC, date_to DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['date_to'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['project_id'];?>">
