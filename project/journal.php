...
$stmt = $conn->prepare("INSERT INTO `publication_in_journal`(`user_id`, `title_of_article`, `with_or_without_issn`, `journal_name`, `level`, `author`, `type`, `impact_factor`, `citation_index`, `h_index`, `scopus_index`, `issn`, `volume`, `issue_no`, `page_numbers`, `date_of_publish`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("isssssssssssssss", $user_id, $title, $issn, $name, $level, $author, $type,$factor,$citation,$hindex,$sindex,$pissn,$volume,$ino,$pno,$publish);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `publication_in_journal` SET `title_of_article`= ?,`with_or_without_issn`=?,`journal_name`=?,`level`=?,`author`=?,`type`=?,`impact_factor`=?,`citation_index`=?,`h_index`=?,`scopus_index`=?,`issn`=?,`volume`=?,`issue_no`=?,`page_numbers`=?,`date_of_publish`=?
            WHERE `user_id`=? AND `journal_id`=?");

        $updateStmt->bind_param("sssssssssssssssii",$title, $issn, $name, $level, $author, $type,$factor,$citation,$hindex,$sindex,$pissn,$volume,$ino,$pno,$publish,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `publication_in_journal` WHERE `user_id`=? AND `journal_id`=?");
...
<br>
                  ...
                  <center><h3>Publication In Journal</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
		...
		<div class="col-md-6">
		  ...
		  <br>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" id="withissn" name="issn" value="withISSN">
              <label class="form-check-label" for="withissn">with ISSN</label>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" id="withoutissn" name="issn" value="withoutISSN">
              <label class="form-check-label" for="withoutissn">without ISSN</label>
            </div>
          </div><br><br>
        </div>
		<div class="col-md-6">
		...
		<div class="col-md-3">
          <div class="form-group">
            <label for="factor">Impact Factor</label>
            <input name="factor" type="text" class="form-control" id="factor">
			<br><br>
          </div>
        </div>
		<div class="col-md-3">
          <div class="form-group">
            <label for="citation">Citation Index</label>
		...
		<div class="col-md-3">
          <div class="form-group">
            ...
            <input name="hindex" type="text" class="form-control" id="hindex">
			<br><br>
          </div>
        </div>
		<div class="col-md-4">
          <div class="form-group">
            <label for="sindex">Scopus Index</label>
            <input name="sindex" type="text" class="form-control" id="sindex">
			<br><br>
          </div>
        </div>
		<div class="col-md-4">
          <div class="form-group">
            <label for="pissn">ISSN</label>
            <input name="pissn" type="text" class="form-control" id="pissn">
			<br><br>
          </div>
        </div>
		<div class="col-md-4">
		...
		<div class="col-md-4">
          <div class="form-group">
            <label for="publish">Date Of Publish</label>
            <input name="publish" type="text" class="form-control" id="publish" required>
			<br><br>
          </div>
        </div>
		<div class="row">
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="book.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM publication_in_journal WHERE user_id = '$user_id' ORDER BY date_of_publish DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['with_or_without_issn'];?></td>
                    <td><?=$row['journal_name'];?></td>
                    <td><?=$row['level'];?></td>
                    <td><?=$row['author'];?></td>
                    <td><?=$row['type'];?></td>
                    <td><?=$row['impact_factor'];?></td>
                    <td><?=$row['citation_index'];?></td>
                    <td><?=$row['h_index'];?></td>
                    <td><?=$row['scopus_index'];?></td>
                    <td><?=$row['issn'];?></td>
                    <td><?=$row['volume'];?></td>
                    <td><?=$row['issue_no'];?></td>
                    <td><?=$row['page_numbers'];?></td>
                    <td><?=$row['date_of_publish'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['journal_id'];?>">
  ...
  <div class="modal-dialog modal-dialog-centered modal-xl">
    ...
    <div class="col-md-6">
          <div class="form-group">
            ...
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" id="withissn" name="issn" value="withISSN">
              <p class="form-check-label">with ISSN</p>
            </div>
            <div class="form-check form-check-inline">
              <input class="form-check-input" type="radio" id="withoutissn" name="issn" value="withoutISSN">
              <p class="form-check-label">without ISSN</p>
            </div>
            <br>
          </div>
        </div>
