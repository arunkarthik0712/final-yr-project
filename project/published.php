...
$stmt = $conn->prepare("INSERT INTO `books_published`(`user_id`, `with_or_without_isbn`, `title_of_book`, `level_of_book`, `author`, `published_name`, `edition`, `ch_in_edited_book`, `isbn`, `place_of_publication`, `month_and_year_of_publication`) VALUES (?,?,?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("issssssssss", $user_id, $isbn, $name, $level, $author, $pubname,$edition,$edited,$pisbn,$place,$month);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `books_published` SET `with_or_without_isbn`=?,`title_of_book`=?,`level_of_book`=?,`author`=?,`published_name`=?,`edition`=?,`ch_in_edited_book`=?,`isbn`=?,`place_of_publication`=?,`month_and_year_of_publication`=?
            WHERE `user_id`=? AND `publication_id`=?");

        $updateStmt->bind_param("ssssssssssii",$isbn, $name, $level, $author, $pubname,$edition,$edited,$pisbn,$place,$month,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `books_published` WHERE `user_id`=? AND `publication_id`=?");
...
<br>
                  ...
                  <center><h3>Books Published/Edited</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
		...
		<div class="col-md-4">
          <div class="form-group">
            <label for="level">Level Of The Book</label>
        ...
        <div class="col-md-3">
          <div class="form-group">
            <label for="edited">CH. In Edited Book</label>
            <input name="edited" type="text" class="form-control" id="edited">
            <br><br>
          </div>
        </div>
		<div class="col-md-2">
		...
		<div class="col-md-3">
          <div class="form-group">
            <label for="place">Place - Publication</label>
		...
		<div class="col-md-4">
          <div class="form-group">
            <label for="month">Month & Year Of Publication</label>
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="conf_wrk_sem_detail.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
                  <tr>
                    ...
                    <th>Edition</th>
                    <th>CH.In_Edited_book</th>
                    <th>Isbn</th>
                    <th>Place_publication</th>
                    <th>Month_&_Year_Of_Publication</th>
                    <th>Action</th>
                  </tr>
                </thead>
          <tbody>
              <?php
              $query = "SELECT * FROM books_published WHERE user_id = '$user_id' ORDER BY month_and_year_of_publication DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['title_of_book'];?></td>
                    <td><?=$row['level_of_book'];?></td>
                    <td><?=$row['author'];?></td>
                    <td><?=$row['published_name'];?></td>
                    <td><?=$row['edition'];?></td>
                    <td><?=$row['ch_in_edited_book'];?></td>
                    <td><?=$row['isbn'];?></td>
                    <td><?=$row['place_of_publication'];?></td>
                    <td><?=$row['month_and_year_of_publication'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['publication_id'];?>">
