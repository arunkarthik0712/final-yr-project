...
$stmt = $conn->prepare("INSERT INTO `work_experience`(
    `user_id`, `institution_name`, `designation`, `address_of_institution`, `pay_band_basic_pay`,
    `period_of_service_from`, `period_of_service_to`) VALUES (?,?,?,?,?,?,?)");


$stmt->bind_param("issssss", $user_id, $name, $desg, $address, $pay, $from, $todate);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE work_experience SET 
            `institution_name`=?, `designation`=?, `address_of_institution`=?, `pay_band_basic_pay`=?,
    `period_of_service_from`=?, `period_of_service_to`=?
            WHERE `user_id`=? AND `experience_id`=?");

        $updateStmt->bind_param("ssssssii", $name, $desg, $address, $pay,$from,$todate,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `work_experience` WHERE `user_id`=? AND `experience_id`=?");
...
<br>
                  ...
                  <center><h3>Work Experience</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <input name="desg" type="text" class="form-control" id="desg" required >
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="address">Address of the  Institution</label>
            <textarea class="form-control" name="address" id="address"></textarea>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="pay">Pay Band/Basic Pay</label>
            <input name="pay" type="text" class="form-control" id="pay">
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="from">Period Of Service From</label>
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <input name="todate" type="text" class="form-control" id="todate" >
            <br><br>
          </div>
        </div>
        <div class="row">
        <div class="col-md-6">
          <div class="form-group">
            ...
            <a href="exp.php" class="btn btn-dark" role="button">Next</a>
          </div>
        </div>
        </div>
    </div>
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
                  <tr>
                    ...
                    <th>Address_of_the_Institution</th>
                    <th>Designation</th>
                    <th>Basic_pay</th>
                    <th>From</th>
                    <th>To</th>
              ...
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td><?=$row['address_of_institution'];?></td>
                    <td><?=$row['designation'];?></td>
                    <td><?=$row['pay_band_basic_pay'];?></td>
                    <td><?=$row['period_of_service_from'];?></td>
                    <td><?=$row['period_of_service_to'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['experience_id'];?>">
  ...
  <div class="modal-dialog modal-dialog-centered modal-xl">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="col-md-4">
          <div class="form-group">
            <p>Address of the  Institution</p>
            <textarea class="form-control" name="address" id="address"></textarea>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <p>Pay Band/Basic Pay</p>
            <input name="pay" type="text" class="form-control" id="pay">
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <p>Period Of Service From</p>
            <input name="from" type="text" class="form-control" id="from" >
          </div>
        </div>
        <div class="col-md-4">
