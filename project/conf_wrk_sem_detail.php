...
$stmt = $conn->prepare("INSERT INTO `conferences_workshops_seminars_organized`(`user_id`, `conf_workshop_seminar_type`, `level`, `title`, `place`, `sponsors_name`, `date_from`, `date_to`) VALUES (?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("isssssss", $user_id, $conf, $level, $name, $place, $sname,$fdate,$todate);
...
header("Location: conf_wrk_sem_detail.php");
        exit();
$stmt->close();
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `conferences_workshops_seminars_organized` SET `conf_workshop_seminar_type`=?,`level`=?,`title`=?,`place`=?,`sponsors_name`=?,`date_from`=?,`date_to`=?
            WHERE `user_id`=? AND `organized_id`=?");

        $updateStmt->bind_param("sssssssii",$conf, $level, $name, $place, $sname,$fdate,$todate,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `conferences_workshops_seminars_organized` WHERE `user_id`=? AND `organized_id`=?");
...
<br>
                  ...
                  <center><h3>Conferences/Workshops/Seminars Organized With Details</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="form-group">
            <label for="conf">*CONF/WKSHP/SEM</label>
            <select class="form-control" id="conf" name="conf" required>
        ...
        <div class="col-md-3">
          <div class="form-group">
            <label for="sname">Sponsor's name</label>
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="awards.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
                  <tr>
                    <th> </th>
                    <th>Staff_Name</th>
                    <th>conf/wkshop/sem</th>
                    <th>level</th>
                    <th>title</th>
                    <th>place</th>
                    <th>sponsor's_name</th>
                    <th>date_from</th>
                    <th>date_to</th>
                    <th>Action</th>
                  </tr>
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM conferences_workshops_seminars_organized WHERE user_id = '$user_id' ORDER BY date_from DESC, date_to DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    <td><?=$i; ?></td>
                    <td><?=$user['uname'];?></td>
                    <td><?=$row['conf_workshop_seminar_type'];?></td>
                    <td><?=$row['level'];?></td>
                    <td><?=$row['title'];?></td>
                    <td><?=$row['place'];?></td>
                    ...
                    <td><?=$row['date_to'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['organized_id'];?>">
