...
$stmt = $conn->prepare("INSERT INTO `awards_received`(`user_id`, `award_name`, `level`, `instituted_by`, `presented_by`, `amount_received`, `month_and_year`) VALUES (?,?,?,?,?,?,?)");


$stmt->bind_param("issssss", $user_id, $name, $level, $institute, $present, $amount,$month);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `awards_received` SET `award_name`=?,`level`=?,`instituted_by`=?,`presented_by`=?,`amount_received`=?,`month_and_year`=?
            WHERE `user_id`=? AND `award_id`=?");

        $updateStmt->bind_param("ssssssii",$name, $level, $institute, $present, $amount,$month,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `awards_received` WHERE `user_id`=? AND `award_id`=?");
...
<br>
                  ...
                  <center><h3>Awards Recevied With Details</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        <div class="col-md-5">
          <div class="form-group">
            <label for="name">*Name Of The Award</label>
        ...
        <div class="col-md-4">
          <div class="form-group">
            <label for="institute">Instituted By</label>
        ...
        <div class="col-md-4">
          <div class="form-group">
            <label for="present">Presented By</label>
            <input name="present" type="text" class="form-control" id="present" >
            <br><br>
          </div>
        </div>
        <div class="col-md-4">
          <div class="form-group">
            <label for="amount">Amount Received(If Any)</label>
		...
		<div class="col-md-4">
          <div class="form-group">
            ...
            <input name="month" type="date" class="form-control" id="month">
            <br><br>
          </div>
        </div>
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="reproject.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM awards_received WHERE user_id = '$user_id' ORDER BY month_and_year DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['award_id'];?>">
