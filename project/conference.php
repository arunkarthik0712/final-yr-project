...
$stmt = $conn->prepare("INSERT INTO `conference_workshop`(`user_id`, `conf_workshop_type`, `conf_workshop_name`, `sponsored_by`, `organized_by`, `place`, `level`, `date_from`, `date_to`) VALUES (?,?,?,?,?,?,?,?,?)");


$stmt->bind_param("issssssss", $user_id, $conf, $name, $sponsored, $organized, $place, $level,$fdate,$todate);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `conference_workshop` SET `conf_workshop_type`=?,`conf_workshop_name`=?,`sponsored_by`=?,`organized_by`=?,`place`=?,`level`=?,`date_from`=?,`date_to`=?
            WHERE `user_id`=? AND `conf_workshop_id`=?");

        $updateStmt->bind_param("ssssssssii",$conf, $name, $sponsored, $organized, $place, $level,$fdate,$todate,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `conference_workshop` WHERE `user_id`=? AND `conf_workshop_id`=?");
...
<br>
                  ...
                  <center><h3>Conference/Workshop</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="form-group">
            <label for="name">*CONF/WKSHP/SEM NAME</label>
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="paper.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM conference_workshop WHERE user_id = '$user_id' ORDER BY date_from DESC, date_to DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    <td><?=$i; ?></td>
                    <td><?=$user['uname'];?></td>
                    <td><?=$row['conf_workshop_type'];?></td>
                    <td><?=$row['conf_workshop_name'];?></td>
                    ...
                    <td><?=$row['date_to'];?></td>
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['conf_workshop_id'];?>">
