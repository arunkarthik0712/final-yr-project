...
$stmt = $conn->prepare("INSERT INTO `additional`(`user_id`, `additional_info`) VALUES (?,?)");
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `additional` SET `additional_info`=? WHERE `user_id`=? AND `add_id`=?");

        $updateStmt->bind_param("sii",$message,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `additional` WHERE `user_id`=? AND `add_id`=?");
...
<br>
                  ...
                  <center><h3>Any Additional Information,Specify</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
              ...
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['add_id'];?>">
