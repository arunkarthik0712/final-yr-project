...
$stmt = $conn->prepare("INSERT INTO `resource_person`(`user_id`, `topic`, `level`, `type`, `place`, `date_of_event`) VALUES (?,?,?,?,?,?)");


$stmt->bind_param("isssss", $user_id, $topic, $level, $type, $place, $date);
...
if (isset($_POST['btn1'])) {
        ...
        $updateStmt = $conn->prepare("UPDATE `resource_person` SET `topic`=?,`level`=?,`type`=?,`place`=?,`date_of_event`=? WHERE `user_id`=? AND `resource_id`=?");

        $updateStmt->bind_param("sssssii",$topic, $level, $type, $place, $date,$user_id, $update_course_id);
...
//delete
 if (isset($_POST['btn2'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM `resource_person` WHERE `user_id`=? AND `resource_id`=?");
...
<br>
                  ...
                  <center><h3>Resource Person Details</h3></center><br/>
                  <hr>
                  <br/> 
                     </div>
                      <div class="row">
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          <div class="form-group">
			...
			<a href="additional.php" class="btn btn-dark" role="button">Next</a>
			<br>
          </div>
        </div>
        </div>
      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-hover table-sm" id="conferenceTable">
         <thead class="table-primary">
          ...
          <tbody>
              <?php
              $query = "SELECT * FROM resource_person WHERE user_id = '$user_id' ORDER BY date_of_event DESC";
              $result = $conn->query($query);
              $i=1;
              if (mysqli_num_rows($result)>0) {
                foreach ($result as $row) {
                  ...
                  <tr>
                    ...
                    <td>
                      <input type="hidden" name="selected_id" value="<?=$row['resource_id'];?>">
